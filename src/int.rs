...
use crate::{Bounded, CtChoice, Encoding, Limb, NonZero, Uint, Word, Zero};
...
impl<const LIMBS: usize> Int<LIMBS> {
    ...
    pub const MIN: Self = Self(Uint::MAX.bitxor(&Uint::MAX.shr(1usize))); // Bit sequence (be): 1000....0000

    /// Maximum value this [`Int`] can express.
    pub const MAX: Self = Self(Uint::MAX.shr(1usize)); // Bit sequence (be): 0111....1111
    ...
    pub const BITS: usize = Uint::<LIMBS>::BITS;
    ...
    pub fn to_nz(self) -> CtOption<NonZero<Self>> {
        CtOption::new(NonZero(self), self.0.ct_is_nonzero().into())
    }
...
mod tests {
    ...
    fn is_minimal() {
        ...
        assert_eq!(min.is_min().to_u8(), CtChoice::TRUE.to_u8());

        let random = I128::from_be_hex("11113333555577779999BBBBDDDDFFFF");
        assert_eq!(random.is_min().to_u8(), CtChoice::FALSE.to_u8());
    }

    #[test]
    fn is_maximal() {
        let max = I128::from_be_hex("7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
        assert_eq!(max.is_max().to_u8(), CtChoice::TRUE.to_u8());

        let random = I128::from_be_hex("11113333555577779999BBBBDDDDFFFF");
        assert_eq!(random.is_max().to_u8(), CtChoice::FALSE.to_u8());
    }
