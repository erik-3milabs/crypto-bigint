...
impl<const LIMBS: usize> Int<LIMBS> {
    ...
    fn div_rem_base(
        &self,
        rhs: &NonZero<Self>,
    ) -> (Uint<{ LIMBS }>, Uint<{ LIMBS }>, CtChoice, CtChoice) {
        ...
        let (quotient, remainder) = lhs_mag.div_rem(&NonZero::<Uint<LIMBS>>::const_new(rhs_mag).0);
    ...
    pub fn checked_div_floor(&self, rhs: &Self) -> CtOption<Self> {
        NonZero::new(*rhs).and_then(|rhs| {
            ...
            let opposing_signs = lhs_sgn.ne(rhs_sgn);
            let increment_quotient = remainder.ct_is_nonzero().and(opposing_signs);
    ...
    pub fn checked_div_mod_floor(&self, rhs: &Self) -> CtOption<(Self, Self)> {
        ...
        NonZero::new(rhs_mag).and_then(|rhs_mag| {
            ...
            let modify = remainder.ct_is_nonzero().and(opposing_signs);
            let quotient_sub_one = quotient.wrapping_add(&Uint::ONE);
