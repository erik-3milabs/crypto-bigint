...
use crate::{Checked, CheckedMul, ConcatMixed, CtChoice, Int, Uint, Zero};
...
impl<const LIMBS: usize> Int<LIMBS> {
    ...
    pub const fn split_mul<const RHS_LIMBS: usize>(
        &self,
        rhs: &Int<RHS_LIMBS>,
    ) -> (Uint<{ LIMBS }>, Uint<{ RHS_LIMBS }>, CtChoice) {
        ...
        let (lo, hi) = lhs_abs.mul_wide(&rhs_abs);
        ...
        Uint<LIMBS>: ConcatMixed<Uint<RHS_LIMBS>, MixedOutput=Uint<WIDE_LIMBS>>,
    {
        ...
        // always fits
        Int::new_from_uint(product_abs.conditional_wrapping_neg(product_sign))
    }
}
...
impl<const LIMBS: usize, const HLIMBS: usize> Mul<Checked<Int<HLIMBS>>> for Checked<Int<LIMBS>> {
    type Output = Self;

    fn mul(self, rhs: Checked<Int<HLIMBS>>) -> Checked<Int<LIMBS>> {
        Checked(self.0.and_then(|a| rhs.0.and_then(|b| a.checked_mul(&b))))
    }
}

impl<const LIMBS: usize, const HLIMBS: usize> Mul<&Checked<Int<HLIMBS>>> for Checked<Int<LIMBS>> {
    type Output = Checked<Int<LIMBS>>;
...
impl<const LIMBS: usize, const HLIMBS: usize> Mul<&Checked<Int<HLIMBS>>>
for &Checked<Int<LIMBS>>
{
    type Output = Checked<Int<LIMBS>>;
