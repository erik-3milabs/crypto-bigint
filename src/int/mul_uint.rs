...
use crate::{CheckedMul, ConcatMixed, CtChoice, Int, Uint, Zero};
...
impl<const LIMBS: usize> Int<LIMBS> {
    ...
    pub const fn split_mul_uint<const RHS_LIMBS: usize>(
        ...
        let (lo, hi) = lhs_abs.mul_wide(rhs);
    ...
    pub fn widening_mul_uint<const RHS_LIMBS: usize, const WIDE_LIMBS: usize>(
        &self,
        rhs: &Uint<RHS_LIMBS>,
    ) -> Int<WIDE_LIMBS>
    where
        Uint<LIMBS>: ConcatMixed<Uint<RHS_LIMBS>, MixedOutput=Uint<WIDE_LIMBS>>,
    {
        ...
        // always fits
        Int::new_from_uint(product_abs.conditional_wrapping_neg(lhs_sign))
    }
}
