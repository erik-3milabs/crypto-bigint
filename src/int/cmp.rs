...
impl<const LIMBS: usize> Int<LIMBS> {
    ...
    pub(crate) const fn gt(lhs: &Self, rhs: &Self) -> CtChoice {
        Uint::ct_gt(&lhs.invert_msb().0, &rhs.invert_msb().0)
    }

    /// Returns the truthy value if `self >= rhs` and the falsy value otherwise.
    #[inline]
    pub(crate) const fn gte(lhs: &Self, rhs: &Self) -> CtChoice {
        Self::lt(lhs, rhs).not()
    }
    ...
    pub(crate) const fn cmp(lhs: &Self, rhs: &Self) -> i8 {
        Uint::ct_cmp(&lhs.invert_msb().0, &rhs.invert_msb().0)
    }
...
mod tests {
    ...
    #[test]
    fn test_is_nonzero() {
        assert_eq!(I128::MAX.ct_is_nonzero().to_u8(), 1u8);
        assert_eq!(I128::ONE.ct_is_nonzero().to_u8(), 1u8);
        assert_eq!(I128::ZERO.ct_is_nonzero().to_u8(), 0u8);
        assert_eq!(I128::MINUS_ONE.ct_is_nonzero().to_u8(), 1u8);
        assert_eq!(I128::MAX.ct_is_nonzero().to_u8(), 1u8);
    }
