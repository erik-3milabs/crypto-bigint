...
use crate::{CtChoice, Int, NonZero, Uint};
...
impl<const LIMBS: usize> Int<LIMBS> {
    ...
    fn div_rem_base_uint(
        &self,
        rhs: &NonZero<Uint<LIMBS>>,
    ) -> (Uint<{ LIMBS }>, Uint<{ LIMBS }>, CtChoice) {
    ...
    /// assert_eq!(remainder, I128::from(-2));
    /// ```
    pub fn div_rem_uint(&self, rhs: &NonZero<Uint<LIMBS>>) -> (Self, Self) {
        let (quotient, remainder, lhs_sgn) = self.div_rem_base_uint(rhs);
        (
            Self(quotient).conditional_wrapping_neg(lhs_sgn),
            Self(remainder).conditional_wrapping_neg(lhs_sgn),
        )
    }
    ...
    pub fn div_floor_uint(&self, rhs: &Uint<LIMBS>) -> CtOption<Self> {
        NonZero::new(*rhs).and_then(|rhs| {
            ...
            let increment_quotient = remainder.ct_is_nonzero().and(lhs_sgn);
            let quotient_sub_one = quotient.wrapping_add(&Uint::ONE);
            let quotient = Uint::ct_select(&quotient, &quotient_sub_one, increment_quotient);
    ...
    pub fn div_rem_floor_uint(&self, rhs: &Uint<LIMBS>) -> CtOption<(Self, Self)> {
        ...
        NonZero::new(*rhs).and_then(|rhs| {
            ...
            let modify = remainder.ct_is_nonzero().and(lhs_sgn);
            let quotient_sub_one = quotient.wrapping_add(&Uint::ONE);
            let quotient = Uint::ct_select(&quotient, &quotient_sub_one, modify);
            ...
            let remainder = Uint::ct_select(&remainder, &inv_remainder, modify);
