...
impl<const LIMBS: usize, const UNSAT_LIMBS: usize> InvMod for Int<LIMBS>
where
    Odd<Uint<LIMBS>>: PrecomputeInverter<Inverter = SafeGcdInverter<LIMBS, UNSAT_LIMBS>>,
{
    type Modulus = NonZero<Uint<LIMBS>>;
    type Output = Uint<LIMBS>;

    fn inv_mod(&self, modulus: &Self::Modulus) -> CtOption<Self::Output> {
        let (abs, sgn) = self.abs_sign();
